...
class RegForm(forms.Form):
    ...
    password = forms.CharField(
        ...
        required=True,
        min_length=6,
        widget=forms.PasswordInput(attrs={'class':'form-control','placeholder':"请输入6位以上密码"})
    )
...
class BindEmailForm(forms.Form):
    ...
    def clean_email(self):
        email = self.cleaned_data['email']
        if User.objects.fitler(email=email).exists():
...
class ForgotPasswordForm(forms.Form):
    ...
    new_password = forms.CharField(
        ...
        required=True,
        min_length=6,
        widget=forms.PasswordInput(attrs={'class':'form-control','placeholder':"请输入6位以上新密码"})
    )
